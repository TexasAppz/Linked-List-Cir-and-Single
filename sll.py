...
# Description2: Singly Linked List
...
class LinkedList:
    ...
    def remove_at_index(self, index: int) -> None:
        ...
        TODO: Write this implementation
        """
        if self.tail == None or index < 0 or self.head.next == self.tail or index >= self.length():
            raise  SLLException()
    ...
    def slice(self, start_index: int, size: int) -> object:
        ...
        if size < 0 or (start_index + size) > n:
            raise SLLException()
        ...
        for i in range(size):
            # adding current value of node to result and advancing node
            result.add_back(node.value)
            node = node.next
        # returning result
        return result
